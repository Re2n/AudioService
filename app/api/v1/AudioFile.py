...
audio_file_router = APIRouter(prefix="/api/v1", tags=["Audio file"])
...
async def audio_file_upload(
    ...
    if not mime_type.startswith("audio/"):
        raise HTTPException(
            status_code=400, detail="Загруженный файл не является аудиофайлом"
        )
    await file.seek(0)
    ...
    audio_file = await audio_file_service.create(
        session, user.id, filename, str(file_location)
    )
